...
class Error {
public:
    Error(int newErrorCode, string newMessage);
    ~Error();
    int errorCode;
    string message;
};
...
public:
	...
	double average;
private:
	void fileToArray(char* filename) throw(Error);
};
...
int main(int argc, char* argv[]) {
    char badFilename[] = "notAFilename";
    //Signal newSig(badFilename); //triggers no file found exception
	//test_addition(); //triggers exception for adding different length Signals
    try{
        int i;
        Signal* signalObject;
        int fileNumber;
                    ...
                    case 's':{
                        ...
                        try{
                            *signalObject * scaleValue;
                            ...
                            cout << endl << "Integer thrown: " << thrownInt << endl;
                        }
                        i++;
                        break;
                    }
        ...
        cout << endl << "Error code: " << e.errorCode << endl;
    }
	return EXIT_SUCCESS;
}
...
void Signal::fileToArray(char* filename) throw(Error){
	FILE *fp;
	int x=0;
	double temp;
	fp=fopen(filename, "r");
    if(fp == NULL){
        throw Error(1, "Error: File not found");
    }
...
Signal operator+(Signal sig1, Signal sig2){
	...
	if(sig1.length != sig2.length){
		throw(Error(2, "Error: Signal lengths don't match and can't be added."));
	} else {
...
Error::Error(int newErrorCode, string newMessage){
